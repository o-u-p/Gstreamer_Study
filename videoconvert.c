...
#define DEFAULT_EFFECTS "identity,test,exclusion,navigationtest," \
    "agingtv,videoflip,vertigotv,gaussianblur,shagadelictv,edgetv"
#define DEFAULT_SOURCE "rtmp://202.69.69.180:443/webcast/bshdlive-pc,rtmp://media3.sinovision.net:1935/live/livestream"
...
main (int argc, char **argv)
{
  ...
  GError *err = NULL;
  GMainLoop *loop;
  GstElement *src, *q1, *q2, *effect, *filter1, *filter2, *sink, *test;
  ...
  if (opt_effects != NULL)
    ...
    effect_names = g_strsplit (DEFAULT_SOURCE, ",", -1);

  for (e = effect_names; e != NULL && *e != NULL; ++e) {
    GstElement *el;

    el = gst_element_factory_make ("source", NULL);
    if (el) {
  ...
  test = gst_element_factory_make("test", NULL);

  gst_bin_add_many (GST_BIN (pipeline), src, filter1, q1, conv_before, effect,
      conv_after, q2, sink, test, NULL);
